...
<Tabs groupId="packageManager" items={['npm', 'pnpm', 'yarn', 'bun']} persist>
  <Tab value="npm">```bash npm install @bprogress/vue ```</Tab>
  <Tab value="pnpm">```bash pnpm add @bprogress/vue ```</Tab>
  <Tab value="yarn">```bash yarn add @bprogress/vue ```</Tab>
  <Tab value="bun">```bash bun add @bprogress/vue ```</Tab>
</Tabs>

Now you are ready to use BProgress in your Vue application!

<Callout type="warn" title="Warning">
  The `@bprogress/vue` package is a **utility** that lets you easily manage a
...
<Cards>
  <Card
    external
    title="NPM"
    href="https://www.npmjs.com/package/@bprogress/vue"
    icon={<SiNpm />}
  >
  ...
  <Card
    external
    title="GitHub"
    href="https://github.com/imskyleen/bprogress/tree/main/packages/vue"
    icon={<FaGithub />}
  >
